...
const solution = (board,moves)=>{
    ...
    moves.forEach((move)=>{
        ...
        const rowIdx = col.findIndex(c=>c !== 0);
        if (rowIdx > -1) {
...
- 2차원배열을 돌다보니, 복잡할 수 밖에 없는 로직이다. 
- rowIdx와 colIdx를 바꿔써서 헤맸다. 디버깅을 잘 해가면서 코딩할 것.
- reduce로 바꿀 수도 있을 것 같다. 
...
```javascript
 const solution = (board,moves)=>{
    const stack = [];
    return moves.reduce((deleteCnt,move)=>{
        const colIdx = move - 1;
        const col = board.map(row=>row[colIdx]);
        const rowIdx = col.findIndex(c=>c !== 0);
        if (rowIdx > -1) {
